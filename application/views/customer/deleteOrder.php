...
if ($_GET['delTop']) {
    $hapusTopping = $_GET['delTop'];
    function hapus($hapusTopping)
    {
        $koneksi = mysqli_connect("localhost", "root", "", "bobaho");
        ...
        $updateHarga = "UPDATE `membeli` SET total_harga = harga * jumlah_pesanan WHERE `membeli`.`id_customer` = '" . $idCustomer['id_customer'] . "' AND `membeli`.`id_cart` = '" . $hapusTopping . "';";
